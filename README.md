...
- 🤔 **Best Practice와 클린 코드 지향**: Best Practice와 [클린코드](https://github.com/qkraudghgh/clean-code-javascript-ko#clean-code-javascript)를 지향하며 근본적인 문제 해결에 집중합니다. 작성한 코드에 만족하지 못한다면 관련 자료를 찾아 더욱 나은 코드를 작성하려고 노력합니다.
...
기능을 개발할 때 **시간을 효율적으로 사용**하기 위해 노력합니다. 사전 미팅 직후에 디자인이나 API가 나오기 전까지 어느 정도 시간이 걸리게 됩니다. 이 때, 먼저 할 수 있는 것들을 미리 구현하거나 출퇴근 시간을 이용하여 관련 자료를 찾아봅니다. 또한 기능 개발 이후에 고도화할 부분들을 메모해두고, 놓치지 않고 개선해나갑니다. 바쁜 일정 속에서도 디자인과 기능을 모두 신경 쓰기 위해 노력하며, UI/UX를 개선하는 데 적극적으로 참여합니다.

필요한 **요청은 항상 의도와 함께 전달**합니다. 이를 통해 좀 더 정확한 피드백을 받을 수 있기 때문에 커뮤니케이션 비용 절감이 가능합니다. 또한 반대로, 요청을 받았을 때도 무조건적으로 받아들이기 보단 의도를 묻고 더 정확한 피드백을 주기 위해 노력합니다.

개발 과정에서 내가 작성한 코드나 구현한 화면 등을 **동료와 공유**하며 성취감을 느낍니다. 개발 트렌드, 컨퍼런스 정보, 소소한 팁 등을 공유하며 지식을 전파합니다. 단순히 링크만 공유하는 것이 아니라 짤막하게나마 제 생각이나 느낌을 덧붙여 공유합니다.
...
[웹 컴포넌트](https://developer.mozilla.org/ko/docs/Web/API/Web_components) 기반의 디자인 시스템을 유지보수한 경험이 있습니다. 프론트엔드 프레임워크 종류에 상관없이 사용할 수 있는 웹 컴포넌트의 장점을 살리면서도 사용자 경험을 향상시키는 데 주력했습니다.

- **폼 요소 개선**: 웹 컴포넌트의 button 요소가 네이티브 동작과 다르다는 점을 발견하고, [웹킷 블로그](https://webkit.org/blog/13711/elementinternals-and-form-associated-custom-elements/)를 참고하여 네이티브와 최대한 유사하게 동작하도록 개선하였습니다.
